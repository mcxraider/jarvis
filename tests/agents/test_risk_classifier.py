...
class TestPartitionToolCalls:
    def test_all_safe(self):
        ...
        risky, safe = partition_tool_calls(calls)
        assert risky == []
        assert len(safe) == 2
    ...
    def test_empty_list(self):
        risky, safe = partition_tool_calls([])
        assert risky == []
        assert safe == []
    ...
    def test_small_batch_stays_safe(self):
