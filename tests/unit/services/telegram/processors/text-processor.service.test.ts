...
describe('TextProcessorService', () => {
  ...
  it('preserves fresh image batches and rejects cumulative overflow without resuming', async () => {
    ...
    const initialImages = Array.from({ length: 10 }, () => ({ ...image }));
    const agentClient = {
      invoke: jest.fn().mockResolvedValue({
        status: 'interrupted',
    ...
    expect(agentClient.resume).not.toHaveBeenCalled();
    expect(await gateStore.getStatus(buildConversationKey(42, 'telegram:42', 100))).toBe(
  ...
  it('omits thread id for fresh invocations so the agent creates a new thread', async () => {
    ...
    await service.processTextMessage('add milk', 701122767, {
      requestId: 'req_1',
      chatId: 555,
      messageId: 42,
    });
    await service.processTextMessage('add eggs', 701122767, {
      requestId: 'req_2',
      chatId: 555,
      messageId: 43,
    });
  ...
  it('does not create a new thread id while a graph run has not completed', async () => {
    ...
    } = require('../../../../../src/services/telegram/conversation-key');
    const gateKey = buildConversationKey(42, 'telegram:42', 100);
  ...
  it('resumes pending clarifications across processor instances when they share stores', async () => {
    ...
    await createService(firstAgentClient, store, gateStore).processTextMessage(
      'update my task',
      42,
      {
        chatId: 100,
    ...
    await expect(
      createService(secondAgentClient, store, gateStore).processTextMessage(
        'the dentist task',
        42,
        {
          chatId: 100,
  ...
  it('clears pending clarification state after a failed resume', async () => {
    ...
    await expect(service.processTextMessage('show today', 42)).resolves.toHaveProperty(
      'response',
  ...
  it('blocks unrelated text when a confirm interrupt is pending and returns a warning', async () => {
    ...
    const thirdResult = await createService(agentClient, store, gateStore).processTextMessage(
      'no',
      42,
      {
        chatId: 100,
  ...
  it('resumes a confirm interrupt when user types a decline token', async () => {
    ...
    await service.processTextMessage('remove all tasks on tuesday', 42, {
      chatId: 100,
      messageId: 10,
    });
  ...
  it('allows any text to resume a clarify interrupt (unchanged behaviour)', async () => {
    ...
    const result = await service.processTextMessage('add buy milk', 42, {
      chatId: 100,
      messageId: 11,
    });
  ...
  it('preserves pending record when resume throws during clarification handling', async () => {
    ...
    const resultPromise = service.processTextMessage('the dentist task', 42, {
      chatId: 100,
      messageId: 11,
    });
  ...
  describe('consumed clarification presentation', () => {
    ...
    it('surfaces the clarification id when a clarify reply resolves the pause', async () => {
      ...
      const result = await service.processTextMessage('the dentist task', 42, {
        chatId: 100,
        messageId: 11,
      });
    ...
    it('does NOT surface the id when a non-decision is refused on a confirm pause', async () => {
      ...
      const result = await service.processTextMessage('maybe later', 42, {
        chatId: 100,
        messageId: 11,
      });
