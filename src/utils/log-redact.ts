export const SENSITIVE_KEY_PATTERN =
  /(authorization|token|secret|api[_-]?key|password|bot[_-]?token|openai[_-]?api[_-]?key|todoist[_-]?api[_-]?key|telegram[_-]?secret|file_?id)/i;
...
export function redactValue(value: unknown, key = ''): unknown {
  ...
  if (typeof value === 'string') {
    ...
    if (value.includes('data:image/')) return '[REDACTED_IMAGE_DATA_URL]';
    return value;
  }
